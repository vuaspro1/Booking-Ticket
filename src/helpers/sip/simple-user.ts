...
import { Logger, URI } from '@src/lib/sip/lib/core'
import {
	SessionDescriptionHandler,
	SessionDescriptionHandlerOptions,
} from '@src/lib/sip/lib/platform/web/session-description-handler'
import { SimpleUserDelegate, SimpleUserOptions } from '@src/lib/sip/lib/platform/web/simple-user'
import { Transport } from '@src/lib/sip/lib/platform/web/transport'
...
export class SimpleUser {
	/** Delegate. */
	public delegate: SimpleUserDelegate | undefined | any
	...
	public call(
		...
		this.inviter = new Inviter(this.userAgent, target, inviterOptions)

		// Send INVITE
		return this.sendInvite(this.inviter, inviterOptions, inviterInviteOptions).then(() => {
			return
		})
	}

	public refer(referTo: URI | Session, _options: SessionReferOptions = {}) {
		this.inviter.refer(referTo, {
	...
	private initSession(session: Session, referralInviterOptions?: InviterOptions): void {
		...
		this.session.stateChange.addListener((state: SessionState) => {
			...
			console.log(`🚀 ~ file: simple-user.ts ~ line 797 ~ SimpleUser ~ this.session.stateChange.addListener ~ state`, state)
			console.log(
				`🚀 -------------------------------------------------------------------------------------------------------`
			)
			switch (state) {
	...
	private terminate(): Promise<void> {
		...
		console.log(`🚀 -------------------------------------------------------------------------------------------`)
		console.log(`🚀 ~ file: simple-user.ts ~ line 1151 ~ SimpleUser ~ terminate ~ this`, this)
		console.log(`🚀 -------------------------------------------------------------------------------------------`)
		if (!this.session) {
			return Promise.reject(new Error('Session does not exist.'))
		}
